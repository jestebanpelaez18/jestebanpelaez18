<h1 align="center">Hi there👋, I'm Juan !!</h1>

<p align="center"> <img src="https://komarev.com/ghpvc/?username=jestebanpelaez18&label=Profile%20views&color=0e75b6&style=flat" alt="jestebanpelaez18" /> </p>

Software engineer based in Helsinki, originally from Spain. My path started in Mechanical Engineering, where working with data, math, and 
engineering projects gave me my first contact with programming. That curiosity led me to make a full transition into software.

I spent three years at Hive Helsinki, part of the global 42 network, where I studied software engineering through a fully project-based 
curriculum. No lectures, no spoon-feeding, everything built through real projects, peer reviews, and self-directed learning. I developed 
strong foundations across the full stack: systems programming in C and C++, algorithms and data structures, networking, backend development, 
databases, and containerization. That low-level foundation shapes how I approach every problem today.

Since graduating I have been working as a freelance full-stack developer, building a production e-commerce platform for a luxury fragrance brand launching in Helsinki using Next.js, TypeScript, GraphQL, and Shopify. On the side I have been building personal projects in data engineering, including a production-style pipeline processing real-time public transport data from Helsinki with Python, Airflow, PySpark, dbt, and Azure.

Currently looking for roles in Helsinki where I can keep building real software and growing fast. My sweet spot is where backend, data, and engineering fundamentals come together.

---

<h3 align="left">About Me</h3>

- 🏋🏻 Football, gym, running, and snowboarding keep me sane
- 🍳 Cooking is my secret hobby
- 🌊 Happiest by the sea, originally from Málaga, Spain
- ⚡ Eat, sleep, gym, code, repeat

---

<h3 align="left">Notable Projects</h3>

These are some of the notable projects I've worked on:

- [HSL Data Pipeline](https://github.com/jestebanpelaez18/hsl-gtfs-realtime-pipeline): Production-style Bronze to Silver to Gold pipeline with Airflow, PySpark, dbt, PostgreSQL, and Azure. SparkML classifier with AUC 0.92.
- [Arabian Fragrance](https://arabianfragrance.fi): Headless e-commerce platform with Next.js, TypeScript, and Shopify Storefront API via GraphQL.
- [Trascendence](https://github.com/jestebanpelaez18/ft_transcendence): This project is about to create Real-time multiplayer Pong platform., where users could play Pong with others in real-time multiplayer online games. The website included features like user management and authentication, friends lists, user profiles, tournaments, and support for multiple languages. Django was enforced as the backend framework, and PostgreSQL was used as the only database.
- [Ft_IRC](https://github.com/jestebanpelaez18/ft_irc): This project involves creating an Internet Relay Chat (IRC) server and connecting it to an IRC client for reference. With IRSSI serving as the IRC client reference, the objective is to test the implemented IRC server with the referenced client. Additionally, handle multiple connected clients and the communication between client and server, done via TCP/IP.
- [Inception](https://github.com/jestebanpelaez18/42Course-Inception): A project focused on exploring the world of system administration using Docker. This project involves virtualizing and managing multiple Docker images within a personal virtual machine. It aims to deepen the understanding of containerization and enhance skills in managing and orchestrating systems in practical, real-world scenarios.
- [Minishell](https://github.com/jestebanpelaez18/42Course-Minishell.git): This project aims to develop a user-friendly shell, essentially a simplified version inspired by the bash shell. Implemented in C and using Unix system calls, it optimizes processes and file descriptors. In this shell you will be able to use redirections, piping, executable execution, and seamless handling of quotes, environment variables, and shell expansions. Furthermore, it offers versatility by supporting various types of signals, enhancing your command-line experience. 
- [Cub3D](https://github.com/jestebanpelaez18/Cub3d.git): This project is inspired by the world-famous Wolfenstein 3D game, which was the first FPS ever. mplemented in C and employing fundamental algorithms, our endeavor delves into the realm of ray-casting. The objective is to create an immersive view within a maze, challenging you to navigate and discover your path forward

---

<h3 align="left">Stack</h3>

![Languages and Tools](https://skillicons.dev/icons?i=py,ts,js,c,cpp,bash,react,nextjs,nodejs,tailwind,vercel,django,postgres,mysql,docker,git,github,linux,azure,postman,graphql,githubactions,vscode,ai)

**Data:** PySpark · Airflow · dbt · PostgreSQL · Azure  
**Web:** React · Next.js · Node.js · Django · GraphQL · REST APIs  
**Systems:** C · C++ · Linux · Shell Scripting  
**Tools:** Docker · CI/CD · GitHub Actions · Git

---

<h3 align="left">Connect with me:</h3>

<p align="left">
<a href="https://linkedin.com/in/www.linkedin.com/in/juan-esteban-pelaez-hoyos-123465162" target="blank"><img align="center" src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/linked-in-alt.svg" alt="www.linkedin.com/in/juan-esteban-pelaez-hoyos-123465162" height="30" width="40" /></a>
</p>

[Portfolio](https://jestebanpelaez.dev)

---

<p><img align="left" src="https://github-readme-stats.vercel.app/api/top-langs?username=jestebanpelaez18&show_icons=true&locale=en&layout=compact" alt="jestebanpelaez18" /></p>

---

[![Juanes's GitHub stats](https://github-readme-stats.vercel.app/api?username=jestebanpelaez18&show_icons=true&locale=en&layout=compact&show=reviews,discussions_started,discussions_answered,prs_merged,prs_merged_percentage)](https://github.com/anuraghazra/github-readme-stats)


